    ...
    try {
        ...
        const hotelsData = await new Promise(resolve => {
            request(urlRequest, function (error, response, body) {
                ...
                asyncForEach(bodyJson.b_hotels, async (item) => {
                   ...
                   await download(image_url + item.b_image_url, 'images/'+lastName, function () {
                        // console.log('done');
                    });
                   await scrapeIt('https://www.booking.com'+item.b_url, {
                        address: ".js-hp_address_subtitle"
                        , desc: "#summary"
                        // Fetch the blog pages
                        , roomNames: {
                            listItem: "tr.hprt-table-last-row",
                            name : 'roomNames'
                            , data: {
                                title: ".hprt-roomtype-icon-link"
                            }
                        }
                        , avatar: {
                            selector: "#photo_wrapper > div.hp-gallery-slides.hp-gallery-top.slick-initialized.slick-slider > div > div > div.slick-slide.slick-active > img"
                            , attr: "src"
                        }
                    }).then(({ data, response }) => {
        ...
        table.columns.add('description', sql.NVarChar(sql.MAX))
        ...
        table.columns.add('createdBy', sql.DateTime2())
        table.columns.add('createdDate', sql.DateTime2())
        table.columns.add('modifiedBy', sql.VarChar(255))
